...
export default function ProductTreeLinksClient({ products }: Props) {
  ...
  return (
    <div className="grid p-7 grid-cols-1 md:grid-cols-3 xl:grid-cols-3 gap-6">
      {Object.keys(TYPE_LABEL).map((type) => {
        const list = grouped[type] ?? [];
        if (!list.length) return null;
        const ids = list.map((p: any) => p.id).join(",");
        return (
          <div key={type} className="xl:pb-3 md:pb-3">
            <button
              onClick={() => toggle(type)}
              className="flex items-center gap-3"
            >
              <div className="flex items-center gap-3">
                <span className=" text-sm xl:font-medium text-white ">
                  {TYPE_LABEL[type]}
                </span>
              </div>
              <div>
